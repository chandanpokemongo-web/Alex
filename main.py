...
ALL_COMMANDS = [
    ...
    BotCommand("info",         "Show user ID, status and warnings"),
    BotCommand("report",       "Report a message to all admins"),
    BotCommand("stats",        "Show group activity statistics"),
    BotCommand("id",           "Show your Telegram user ID"),
    BotCommand("ping",         "Check bot response time"),
    BotCommand("log",          "View recent admin actions"),
    # Admin tools
    BotCommand("ban",          "Ban a user permanently [admin]"),
    BotCommand("unban",        "Remove a user from ban list [admin]"),
    BotCommand("kick",         "Kick a user (can rejoin) [admin]"),
    BotCommand("mute",         "Mute a user for a duration [admin]"),
    BotCommand("unmute",       "Restore a muted user [admin]"),
    BotCommand("warn",         "Give a user a warning strike [admin]"),
    BotCommand("resetwarns",   "Clear all warnings for a user [admin]"),
    BotCommand("promote",      "Grant admin rights to a user [admin]"),
    BotCommand("demote",       "Remove admin rights from a user [admin]"),
    BotCommand("pin",          "Pin the replied-to message [admin]"),
    BotCommand("unpin",        "Remove the pinned message [admin]"),
    BotCommand("purge",        "Delete messages from reply to now [admin]"),
    BotCommand("lock",         "Lock stickers/polls/media/all [admin]"),
    BotCommand("unlock",       "Remove all chat restrictions [admin]"),
    BotCommand("slowmode",     "Set slow mode delay [admin]"),
    BotCommand("setrules",     "Save group rules text [admin]"),
    BotCommand("setwelcome",   "Set welcome message with photo+buttons [admin]"),
    BotCommand("setgoodbye",   "Set goodbye message when members leave [admin]"),
    BotCommand("utag",         "Tag all group members [admin]"),
    # AFK
    BotCommand("afk",          "Set yourself as Away From Keyboard"),
    BotCommand("unafk",        "Remove your AFK status manually"),
    # Tools
    BotCommand("weather",      "Current weather — /weather Mumbai"),
    BotCommand("define",       "Dictionary definition — /define serendipity"),
    BotCommand("fact",         "Random interesting fact"),
    BotCommand("bmi",          "BMI calculator — /bmi 70 175"),
    BotCommand("age",          "Age calculator — /age 15/08/2000"),
    BotCommand("countdown",    "Days until a date — /countdown 31/12/2025"),
    BotCommand("password",     "Generate secure password — /password 16"),
    BotCommand("number",       "Fun fact about a number — /number 42"),
    BotCommand("roll",         "Dice roller — /roll 2d6"),
    BotCommand("qr",           "Generate QR code — /qr https://t.me/group"),
    BotCommand("calc",         "Calculator — /calc 25*4+10"),
    BotCommand("quote",        "Random motivational quote"),
    BotCommand("remind",       "Set a reminder — /remind 10m check oven"),
    BotCommand("choose",       "Bot picks from your options"),
    BotCommand("flip",         "Random decision between two choices"),
    # Info & lookup tools
    BotCommand("crypto",       "Crypto price — /crypto BTC"),
    BotCommand("ip",           "IP geolocation — /ip 8.8.8.8"),
    BotCommand("urban",        "Urban Dictionary — /urban slay"),
    BotCommand("translate",    "Translate text — /translate hi Hello"),
    BotCommand("lyrics",       "Song lyrics — /lyrics Ed Sheeran - Shape of You"),
    # Unit converters
    BotCommand("temp",         "Temperature convert — /temp 100 C F"),
    BotCommand("weight",       "Weight convert — /weight 10 kg lb"),
    BotCommand("currency",     "Currency convert — /currency 100 USD INR"),
    BotCommand("time",         "Time in timezone — /time IST"),
    BotCommand("color",        "Color info + preview — /color FF5733"),
    # AI image
    BotCommand("image",        "Generate AI image — /image a sunset over Mumbai"),
    BotCommand("link",         "Get AI image URL — /link a samurai at sunset"),
    # Anime image
    BotCommand("waifu",        "Random anime waifu photo"),
    BotCommand("animepic",     "Random anime character photo"),
    # Fun tools
    BotCommand("chuck",        "Random Chuck Norris fact"),
    BotCommand("dadjoke",      "Groan-worthy dad joke"),
    BotCommand("compliment",   "Compliment someone — /compliment @user"),
    BotCommand("roast",        "Roast someone — reply + /roast"),
    BotCommand("slot",         "Slot machine — try for jackpot!"),
    BotCommand("toss",         "Simple coin toss (Heads or Tails)"),
    # Text transform (most popular shown; others work by typing them)
    BotCommand("reverse",      "Reverse text — /reverse hello"),
    BotCommand("mock",         "Spongebob mock — /mock hello"),
    BotCommand("aesthetic",    "Aesthetic text — /aesthetic hello"),
    BotCommand("bold",         "Bold Unicode — /bold hello"),
    BotCommand("binary",       "Text to binary — /binary hi"),
    BotCommand("morse",        "Text to Morse — /morse SOS"),
    BotCommand("wordcount",    "Word & char count — /wordcount text"),
    # Downloads
    BotCommand("download",     "Download video from any platform (up to 4K)"),
    BotCommand("sdownload",    "Download a song as MP3 audio"),
    # Music
    BotCommand("play",         "Play a song (VC stream or audio file)"),
    BotCommand("pause",        "Pause VC playback"),
    BotCommand("resume",       "Resume VC playback"),
    BotCommand("skip",         "Skip to the next song"),
    BotCommand("stop",         "Stop music and leave voice chat"),
    BotCommand("queue",        "Show the music queue"),
    # Games
    BotCommand("trivia",       "Answer first and win a point"),
    BotCommand("hangman",      "Guess the hidden word letter by letter"),
    BotCommand("tictactoe",    "Play Tic-Tac-Toe vs bot or a player"),
    BotCommand("guess",        "Guess the anime character from a photo"),
    BotCommand("pick",         "Choose the correct character from buttons"),
    BotCommand("cricket",      "Guess the cricketer from their photo"),
    BotCommand("playcricket",  "Play hand cricket PvP against another user"),
    BotCommand("rps",          "Rock Paper Scissors vs the bot"),
    BotCommand("dice",         "Roll an animated dice (1-6)"),
    BotCommand("coinflip",     "Flip an animated coin — Heads or Tails"),
    BotCommand("8ball",        "Ask the magic 8-ball a question"),
    BotCommand("truth",        "Get a random truth question"),
    BotCommand("dare",         "Get a random dare challenge"),
    BotCommand("joke",         "Get a random joke"),
    BotCommand("love",         "Love calculator with another user"),
    BotCommand("meme",         "Get a trending meme"),
    BotCommand("poll",         "Create a group poll"),
    BotCommand("ranking",      "Most active message senders"),
    # Collection & Coins
    BotCommand("collection",   "View your collected cricketers & anime characters"),
    BotCommand("coins",        "Check your RB coin balance"),
]
...
def main():
    ...
    app.add_handler(CommandHandler("guess",       fun.guess))
